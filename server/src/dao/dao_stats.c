...
int dao_stats_update_onevn_game(int64_t winner_id, int64_t *player_ids, int *scores, int *eliminated, int player_count) {
    if (!player_ids || !scores || !eliminated || player_count <= 0) return -1;
    ...
    for (int i = 0; i < player_count; i++) {
        int64_t user_id = player_ids[i];
        int is_winner = (winner_id > 0 && user_id == winner_id) ? 1 : 0;
        
        // Update onevn_games and onevn_wins
        // Note: If user_stats table exists, use it; otherwise update users table directly
        const char *sql =
            "UPDATE users SET "
        ...
        char buf_user[32], buf_win[16];
        snprintf(buf_user, sizeof(buf_user), "%ld", user_id);
        snprintf(buf_win, sizeof(buf_win), "%d", is_winner);
